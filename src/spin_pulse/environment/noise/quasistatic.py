...
class QuasistaticNoiseTimeTrace(NoiseTimeTrace):
    """Generates a quasi-static noise time trace for spin-qubit simulations.
    ...
    def __init__(self, T2S, duration, segment_duration, seed: int | None = None):
        """Initializes a quasi-static Gaussian noise trace.
        ...
        random value with zero mean and standard deviation ``sqrt(2) / T2S``.
    ...
    def plot_ramsey_contrast(self, ramsey_duration: int):
        """Plots the analytical and simulated Ramsey contrast.

        The analytical contrast ``exp(-t^2 / T2S^2)`` is plotted together
