...
class WhiteNoiseTimeTrace(NoiseTimeTrace):
    """Generates a white noise time trace.
    ...
    intensity is determined by the coherence time ``T2S`` through the standard
    deviation ``sigma = sqrt(2 / T2S)``. White noise corresponds to a flat
    ...
    def __init__(self, T2S, duration, segment_duration, seed: int | None = None):
        """Initializes a white noise time trace for spin qubit simulations.

        Each value of the trace is drawn independently from a Gaussian
        distribution with zero mean and standard deviation ``sqrt(2 / T2S)``.
    ...
    def plot_ramsey_contrast(self, ramsey_duration):
        """Plots analytical and numerical Ramsey contrast for white noise.
