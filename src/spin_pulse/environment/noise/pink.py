...
def get_pink_noise(segment_duration, seed: int | None = None):
    """Generates a single segment of pink noise using an inverse FFT method.
...
def get_pink_noise_with_repetitions(
    ...
    """Generates pink noise of a given total duration by repeating segments.
...
class PinkNoiseTimeTrace(NoiseTimeTrace):
    """Generates a pink noise time trace with a 1/f power spectral density.
    ...
    def __init__(self, T2S, duration, segment_duration, seed: int | None = None):
        """Creates a pink noise time trace for spin qubit simulations.
    ...
    def plot_ramsey_contrast(self, ramsey_duration):
        """Plots analytical and numerical Ramsey contrast curves.
