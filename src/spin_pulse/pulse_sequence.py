...
class PulseSequence:
    ...
    def to_hamiltonian(self):
        ...
        sequences (for deviations of the qubit Larmor frequency).

        Returns:
            tuple[list[np.ndarray], list[np.ndarray]]:
                - H: list of Hamiltonian matrices.
        ...
        coeff = []
        for i in range(self.n_pulses):
            H_i, coeff_ = self.pulse_instructions[i].to_hamiltonian()
            coeff_i = np.zeros(self.duration)
            coeff_i[
        ...
        if hasattr(self, "time_trace"):
            ...
            H.append(Pauli("Z").to_matrix() / 2)
            coeff.append(self.time_trace)
        return H, coeff
    ...
    def to_dynamical_decoupling(
        self, hardware_specs: HardwareSpecs, mode: str | None = None
    ):
        ...
        Parameters:
            - hardware_specs (HardwareSpecs): Hardware configuration defining
                the dynamical decoupling sequence type.
            - mode (str | None): Type of the DD sequence to apply. If None, the
        ...
        pulse_instructions = []
        assert self.num_qubits == 1, (
            "error dynamically decouple only possible for one qubit sequences"
        )
        for i in range(self.n_pulses):
            if self.pulse_instructions[i].name == "delay":
                pulse_instructions += self.pulse_instructions[
                    i
                ].to_dynamical_decoupling(hardware_specs, mode=mode)
            else:
