...
class ExperimentalEnvironment:
    ...
    Attributes:
        ...
        - noise_type (NoiseType): Type of noise to simulate. Must be "pink", "white", or "coherent".
        - T2 (float): Characteristic time of individual qubits.
        - TJ (float or None): Characteristic time of coupled two-qubit system at maximal J coupling without noise on Larmor frequency.
        ...
        - time_traces_coupling (list[float]): List of time traces for coupling noise for each pair of qubits (if TJ is set).
    ...
    def __init__(
        ...
        Parameters:
            ...
            - noise_type (NoiseType): Type of noise to simulate. Must be "pink", "white", or "coherent".
            - T2 (float): Characteristic time of individual qubits.
            - TJ (float or None): Characteristic time of coupled two-qubit system at maximal J coupling with no noise on Larmor frequency.
            - duration (int): Total duration of the simulation.
            - only_idle (bool): Flag to apply noise only to idle qubits.
            - segment_duration (int): Duration of each noise segment; used to partition the time trace.
            - seed (int or None): seed integer for random number generation. If not specified, no seed used.
        Raises:
            AssertionError: If an invalid noise_type is provided.
        ...
        match noise_type:
            ...
            case NoiseType.COHERENT:
                self.noise_generator = CoherentNoiseTimeTrace
            case _:
    ...
    def generate_time_traces(self):
        """
        Generate noise time traces for each qubit Larmor frequency and J coupling to each pair of qubits if TJ is defined.
        ...
        Effects:
            ...
            - If TJ is set, populate self.time_traces_coupling with one trace per pair of qubits (n-1 traces for n qubits).
        """
        self.time_traces = []
        for _ in range(self.hardware_specs.num_qubits):
            time_trace = self.noise_generator(
                self.T2, self.duration, self.segment_duration, seed=self.seed
            )
        ...
        if self.TJ is not None:
            ...
            for _ in range(self.hardware_specs.num_qubits - 1):
                time_trace = self.noise_generator(
                    self.TJ, self.duration, self.segment_duration, seed=self.seed
                )
    ...
    def __str__(self):
        ...
        summary = [
            ...
            f"  T2 (qubit dephasing): {self.T2}",
            f"  TJ (coupling dephasing): {self.TJ if self.TJ is not None else 'None'}",
