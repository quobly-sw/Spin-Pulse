...
from .utils import gate_to_pulse_sequences
...
class PulseLayer:
    ...
    def to_circuit(self):
        ...
        for pulse_sequence in self.twoq_pulse_sequences:
            ...
            H0 = [np.kron(_, np.eye(2)) for _ in H0]
            H1, coeff1 = self.oneq_pulse_sequences[qubits[1]._index].to_hamiltonian()
            H1 = [np.kron(np.eye(2), _) for _ in H1]
            H += H0 + H1
            coeff += coeff0 + coeff1
    ...
    def attach_dynamical_decoupling(
        self, hardware_specs: HardwareSpecs, mode: Optional[str] = None
    ) -> None:
        """Apply a dynamical decoupling sequence to all one-qubit pulse sequences
            if possible.

        Each one-qubit PulseSequence in the layer is replaced with a new sequence
        obtained by applying a dynamical decoupling transformation according to
        the specified hardware parameters. Two-qubit sequences are left unchanged.

        Parameters:
            hardware_specs (HardwareSpecs): HardwareSpecs class that defines
                the hardware specifications.
            mode (str | None): Name of the dynamical decoupling strategy to use.
                If None, the default strategy from
                ``PulseSequence.to_dynamical_decoupling`` is applied.
