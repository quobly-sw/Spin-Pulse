...
{% set roles = ({"class": "class", "function": "func", "module": "mod", "attribute": "attr", "data": "data"})	%}
Re-exported objects
-------------------

The following objects can be directly imported from {{ obj.name }} even if they
are implemented in submodules.

.. list-table::
   :widths: 30 70

      {% for exp_name in obj.all %}
         {% for inner in obj.children if inner.name == exp_name %}
   * - :py:{{ roles[inner.type] }}:`~{{ inner.obj.get('original_path', inner.id) }}`
     - {{ inner.obj.doc.split('\n')|first }}
         {% endfor %}
      {% endfor %}
...
{% set ctitle = (obj.type|title) ~ " Contents" %}
{{ ctitle }}
{{ "-" * (ctitle|length) }}
