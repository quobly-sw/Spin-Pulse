spin_pulse.pulse_sequence
=========================

.. py:module:: spin_pulse.pulse_sequence
...
.. autoapisummary::
   ...
   spin_pulse.pulse_sequence.PulseSequence
...
.. py:class:: PulseSequence(pulse_instructions)
   ...
   A PulseSequence is an ordered list of PulseInstruction objects that are
   concatenated in time. The class computes the total duration of the
   sequence, the starting time of each constituent instruction, and provides
   utilities for plotting. It allows the reconstruction of Hamiltonians, to
   attach time traces from a noise environment, and to apply dynamical
   ...
   .. attribute:: t_start_relative
      ...
      Starting time of each pulse_instruction
   ...
   .. py:method:: to_hamiltonian()
      ...
      sequences (for deviations of the qubit's Larmor frequency).

      :returns:     - H: list of Hamiltonian matrices.
                    - coeff: list of time-dependent coefficient arrays of length
                      ``duration``.
   ...
   .. py:method:: adjust_duration(duration)
      ...
      example, to align pulse durations across a PulseLayer.

      :param duration: Target total duration for the sequence.
      :type duration: int
   ...
   .. py:method:: attach_time_trace(time_trace, only_idle)
      ...
      :param only_idle: If True, attach the trace only on idle
                        instructions; all active pulses receive zero time trace.
      :type only_idle: bool
   ...
   .. py:method:: to_dynamical_decoupling(hardware_specs, mode = None)
      ...
      dynamical decoupling mode (as defined in the hardware specifications).
      Non-idle instructions are left unchanged. Only single-qubit sequences
      can be dynamically decoupled.

      :param hardware_specs: Hardware configuration defining
                             the dynamical decoupling sequence type.
      :type hardware_specs: HardwareSpecs
      :param mode: Type of the DD sequence to apply. If None, the
                   default mode specified in ``hardware_specs`` is used.
      :type mode: str | None
