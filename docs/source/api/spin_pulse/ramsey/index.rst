...
.. autoapisummary::
   ...
   spin_pulse.ramsey.get_ramsey_circuit
   spin_pulse.ramsey.contrast
   spin_pulse.ramsey.get_average_ramsey_contrast
...
.. py:function:: get_ramsey_circuit(duration, hardware_specs, exp_env = None)
   ...
   Hadamard gate. The circuit is transpiled into the hardware's native gate
...
.. py:function:: contrast(pulse_circuit)

   Compute the population contrast of a Ramsey experiment.

   The pulse-level circuit is converted back to a unitary QuantumCircuit,
...
.. py:function:: get_average_ramsey_contrast(hardware_specs, exp_env, durations)
   ...
   the environment time traces are reinitialized to ensure independence
   between samples.
