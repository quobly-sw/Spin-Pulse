...
.. list-table::
   ...
   * - :py:class:`~spin_pulse.environment.noise.noise_time_trace.NoiseType`
     - Enumerates the supported noise models for spin-qubit simulations.
...
.. py:class:: NoiseTimeTrace(duration)
   ...
   containing the instantaneous noise values ``epsilon(t)`` that affect the qubit.
   ...
   .. py:method:: ramsey_contrast(ramsey_duration)
      ...
      Computes the Ramsey contrast for a qubit subject to this noise trace.

      The qubit is initialized in the equal superposition state
      ``(|0> + |1>) / sqrt(2)`` and evolves under a pure dephasing Hamiltonian
      ``H(t) = omega(t)``. The Ramsey contrast is obtained from the
      accumulated phase ``exp(-i sum_t omega(t))`` over segments of length
      ``ramsey_duration``. The trace is divided into independent experiments
      of equal length and the contrast is averaged over all experiments.
   ...
   .. py:method:: plot_ramsey_contrast(ramsey_duration)
      ...
      Plots the Ramsey contrast computed from the noise trace.
   ...
   .. py:method:: plot(n_max=None)
      ...
      Plots the noise trace up to a specified number of points.
