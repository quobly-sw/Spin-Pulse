...
def test_plot_with_and_without_time_trace(monkeypatch):
    ...
    fig, ax = plt.subplots()
    print(type(fig))
    seq.plot(ax=None, label_gates=False)  # ax=None -> plt.gca() utilisé
    assert isinstance(fig, Figure)
    assert isinstance(ax, Axes)
...
def test_adjust_duration_adds_idle(monkeypatch):
    ...
    # Cas où on allonge (branche if True)
    seq.adjust_duration(5)  # doit ajouter un Idle de durée 6
...
def test_attach_time_trace_only_idle(monkeypatch):
    ...
    time_trace = np.arange(seq.duration, dtype=float)
    # only_idle=True : bruit que sur les "delay"
...
def test_to_dynamical_decoupling_single_qubit(monkeypatch):
    ...
    new_seq = seq.to_dynamical_decoupling(hw, mode="full_drive")

    # La séquence interne de seq doit avoir été remplacée
    # 'delay' de durée 4 -> deux pulses (2+2), puis X
