...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1, NoiseType.COHERENT, CoherentNoiseTimeTrace, 2**4),
    ],
)
...
def test_coupling_traces_created_when_TJ_is_set(num_qubits, TJ):
    """When TJ is provided, coupling noise traces must be generated."""
    hw = dm.DummyHardwareSpecs(num_qubits=num_qubits)
    env = ExperimentalEnvironment(hardware_specs=hw, TJ=TJ)
    assert hasattr(env, "time_traces_coupling")
    assert len(env.time_traces_coupling) == num_qubits - 1  # n-1 coupling traces


@pytest.mark.parametrize(
    "num_qubits, noise_type, T2, TJ, duration, segment_duration, only_idle, J_coupling",
    [
        (2, NoiseType.COHERENT, 150, None, 1024, 512, False, None),
...
def test_str_representation_contains_all_info(
    ...
    expected_strings = [
        ...
        f"  T2 (qubit dephasing): {T2}",
        f"  TJ (coupling dephasing): {TJ}",
        f"  Duration: {duration}",
