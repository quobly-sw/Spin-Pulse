...
from spin_pulse.pulse_circuit import IdleInstruction, PulseCircuit
...
def test_init_with_dynamical_decoupling_applies_to_layers(two_qubits):
    ...
    hw = dm.DummyHardwareSpecs(dynamical_decoupling="full_drive")
    qc = QuantumCircuit(len(two_qubits))

    pc = PulseCircuit(qc, two_qubits, [layerA, layerB], hw, exp_env=None)

    assert getattr(layerA, "applied_dd", None) == "full_drive"
    assert getattr(layerB, "applied_dd", None) == "full_drive"
...
def test_from_circuit_builds_layers_and_calls_constructor():
    ...
    with (
        patch("spin_pulse.pulse_circuit.circuit_to_dag", return_value=fake_dag),
        patch(
            "spin_pulse.pulse_circuit.dag_to_circuit",
            return_value=LayerAsCirc("layer_as_circ"),
        ),
        patch(
            "spin_pulse.pulse_circuit.PulseLayer.from_circuit_layer",
            return_value=dummy_layer_obj,
        ) as mock_from_layer,
        # patch("spin_pulse.pulse_circuit.PulseCircuit") as mock_pc,
    ):
...
def test_from_circuit_strip_measurement():
    ...
    with (
        patch(
            "spin_pulse.pulse_circuit.circuit_to_dag", return_value=fake_dag
        ) as dag_func,
        patch(
            "spin_pulse.pulse_circuit.dag_to_circuit",
            return_value=LayerAsCirc("layer_as_circ"),
...
def test_get_logical_bitstring_transpiled_layout(two_qubits, pulse_layers, hw_no_dd):
    ...
    # Steps:
    # physical_bits = reverse("10") = ["0","1"]
    # permut=[1,0]
    # logical_bits[i] = physical_bits[permut[i]]
    # logical_bits = [physical_bits[1], physical_bits[0]] = ["1","0"]
    # return "".join(logical_bits[::-1]) = join(["0","1"]) = "01"
    assert out == "01"


# def test_get_logical_bitstring_layout_no_transpile(two_qubits, pulse_layers, hw_no_dd):
#     """Test get_logical_bitstring() with a real Layout (no TranspileLayout)."""
#     qc = QuantumCircuit(len(two_qubits))

#     # Build a valid Qiskit Layout that swaps qubits 0 and 1
#     layout = Layout.from_intlist([1, 0], qc.qubits)
#     qc._layout = layout  # simulate what transpiler normally attaches

#     pc = PulseCircuit(qc, two_qubits, pulse_layers, hw_no_dd, exp_env=None)

#     with warnings.catch_warnings(record=True) as w:
#         result = pc.get_logical_bitstring("10")

#     # It should map qubit 0→1, 1→0 ⇒ bitstring becomes "01"
#     assert result == "01"

#     # And the code should have emitted a warning about missing TranspileLayout
#     assert any("not have a TranspileLayout" in str(wi.message) for wi in w)
...
def test_get_logical_bitstring_no_layout(two_qubits, pulse_layers, hw_no_dd):
    ...
    # physical_bits = reverse("10")->["0","1"]
    # logical_bits = physical_bits, so ["0","1"], then join(reversed)= "10"?
    # Let's compute it carefully:
    # physical_bits = ["0","1"]
    # logical_bits = physical_bits
    # return "".join(logical_bits[::-1]) = join(["1","0"]) = "10"
    assert out == "10"
...
def test_averaging_over_samples(two_qubits, pulse_layers, hw_no_dd):
    ...
    # mock tqdm to avoid progress bar noise
    with (
        patch("spin_pulse.pulse_circuit.tqdm", side_effect=lambda x: x),
...
def test_mean_fidelity_and_mean_channel_wrappers(two_qubits, pulse_layers, hw_no_dd):
    ...
    with (
        patch(
            "spin_pulse.pulse_circuit.average_gate_fidelity", return_value=0.9
        ) as mock_fid,
        patch("spin_pulse.pulse_circuit.Operator.from_circuit") as mock_from_op,
        # patch("spin_pulse.pulse_circuit.SuperOp") as mock_superop,
        ...
        # mean_fidelity()
        out_mean_fid = pc.mean_fidelity(dummy_ref_circ, exp_env="ENV")
