...
from spin_pulse.instructions import (
...
# -------------------------------------------------------------------
# Dummies and utilities
# -------------------------------------------------------------------


# class dm.DummyHardwareSpecs:
#     """Hardware specs stub for both Gaussian and Square pulses."""
#     def __init__(self):
#         self.fields = {"x": 1.0, "y": 1.0, "z": 1.0, "Heisenberg": 1.0}
...
def test_square_rotation_init_and_negative_plateau(capsys):
    q = dm.DummyQubit()
    # plateau_duration < 0 triggers print
    _ = SquareRotationInstruction(
        "x", [q], amplitude=1, sign=1, ramp_duration=10, duration=5
    )
...
def test_from_angle_loop_termination(monkeypatch):
    ...
    N_iter = 3
    monkeypatch.setattr("spin_pulse.instructions.rotations.MAX_ITER", N_iter)
    ...
    with (
        ...
        assert isinstance(instr, SquareRotationInstruction)
        assert mock_to_angle.call_count == N_iter + 2
...
def test_gaussian_from_angle_max_iter_triggers_warning(capsys, monkeypatch):
    ...
    with (
        patch("spin_pulse.instructions.rotations.MAX_ITER", 1),
