...
#
#  In our example, we consider that we are given the following hardware properties, which are compatible with spin qubit state-of-the-art capabilities [see eg arxiv:2507.08713](http://arxiv.org/abs/2507.08713)
...
#
# ``SpinPulse`` is parametrized with coherence times $T_2^*$, and $T_J^*$. while here we are given gate fidelities.
# However, we can set the values of such coherence times from reverse engineering, calculating the gate fidelities for different values of $T_2^*$, and $T_J^*$ (which we write ``T2S``, ``TJS`` in our code for simplicity)
#
